...
class Application(tk.Frame):
	...
	def get_data(self):
		...
		if self.id_ == '':
			...
			self.url1 = f'http://hubblesite.org/api/v3/glossary/{self.id_}'
			self.url2 = f'https://api.le-systeme-solaire.net/rest/bodies/{self.id_}'
			
			try:
				self.definition = planetDef(self.url1)
				self.text.delete('1.0',tk.END)
				self.text.insert(tk.END, self.definition)
				self.update()

				if not self.definition == 'No information on this term':
					self.data = planetary_data(self.url2)
